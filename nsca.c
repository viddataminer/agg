...
static void accept_connection(int sock, void *unused){
	...
	if(!hosts_access(&req)){
        ...
        /* log info to syslog facility */
        strcpy(client_ip, inet_ntoa(nptr->sin_addr));
        if(debug==TRUE)
...
static void handle_connection_read(int sock, void *data){
        ...
        /* all of our host names are lower case, so before nagios sees em... */
        for(sptr = host_name; *sptr; sptr++) *sptr = tolower(*sptr);
		...
		if(!strcmp(svc_description,""))
			syslog(LOG_NOTICE,"HOST CHECK -> Host Name: '%s', Return Code: '%d', Output: '%s (%s)'",host_name,return_code,plugin_output, client_ip);
		else
			syslog(LOG_NOTICE,"SERVICE CHECK -> Host Name: '%s', Service Description: '%s', Return Code: '%d', Output: '%s (%s)'",host_name,svc_description,return_code,plugin_output,client_ip);
	        }
...
static int write_check_result(char *host_name, char *svc_description, int return_code, char *plugin_output, time_t check_time){
	...
	if(!strcmp(svc_description,""))
		fprintf(command_file_fp,"[%lu] PROCESS_HOST_CHECK_RESULT;%s;%d;%s (%s)\n",(unsigned long)check_time, host_name,return_code,plugin_output,client_ip);
	else
		fprintf(command_file_fp,"[%lu] PROCESS_SERVICE_CHECK_RESULT;%s;%s;%d;%s (%s)\n",(unsigned long)check_time,host_name,svc_description,return_code,plugin_output,client_ip);
