    ...
    while(1) {
        ...
        for(i=0; i< arg_num; i++) {
            if(!strcmp(args_of_interest[i], "-v")) {
                i++;
                //if(string_contains_pat(nag_struct, args_of_interest[i], &start, &end))
                if(string_contains_pat_looking_at_substrings_of_the_structure(nag_struct, args_of_interest[i]))
                    break;
            } 
